...
it('opens saved request details from the mobile quick action', async () => {
  const user = userEvent.setup()
  ...
  renderLogs({ logs: [savedLog], onOpenDetail })
  await user.click(screen.getByRole('button', { name: 'Request Details' }))
  expect(onOpenDetail).toHaveBeenCalledWith(savedLog)
})

it('disables the mobile request detail action when no payload was saved', () => {
  renderLogs()
  expect(screen.getByRole('button', { name: 'Request Details' })).toBeDisabled()
})
