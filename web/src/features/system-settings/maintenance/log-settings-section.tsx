...
export function LogSettingsSection(props: LogSettingsSectionProps) {
  ...
  const onSubmit = async (values: LogSettingsFormValues) => {
    if (values.LogConsumeEnabled !== props.defaultValues.LogConsumeEnabled) {
    ...
    if (
      values.DetailedLogRetentionDays !==
      props.defaultValues.DetailedLogRetentionDays
    ) {
      await updateOption.mutateAsync({
        key: 'DetailedLogRetentionDays',
        value: values.DetailedLogRetentionDays,
      })
    }
  }
      ...
      <Form {...form}>
        <SettingsForm onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <SettingsControlGroup className='space-y-3'>
            <div>
              <h4 className='text-sm font-medium'>{t('Data retention')}</h4>
              <p className='text-muted-foreground text-sm'>
                {t('Control log retention and clean historical data.')}
              </p>
            </div>
            <FormField
              control={form.control}
              name='DetailedLogRetentionDays'
              render={({ field }) => (
                <FormItem>
