...
.PHONY: all build-web build-web-classic build-all-web build-frontend build-frontend-classic build-all-frontends start-api start-backend dev dev-api dev-api-rebuild dev-web dev-web-classic dev-frontend dev-backend stop reset-setup
...
stop:
	...
	@pkill -f "go run main.go" 2>/dev/null || echo "No api running"
	@pkill -f "vite" 2>/dev/null || echo "No web dev server running"
